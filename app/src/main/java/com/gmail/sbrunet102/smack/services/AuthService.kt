...
import com.gmail.sbrunet102.smack.utilities.*
...
object AuthService {
    ...
    fun loginUser(email: String, password: String, complete: (Boolean) -> Unit) {
        ...
        val loginRequest =
            object : JsonObjectRequest(Method.POST, URL_LOGIN, null, Response.Listener { response ->
                ...
                Log.d("ERROR login", "Could not login user: $error")
                complete(false)
            }) {
                override fun getBodyContentType(): String {
                    return "application/json; charset=utf-8"
    ...
    fun findUserByEmail(context: Context, complete: (Boolean) -> Unit) {
        ...
        val findUserRequest = object :
            JsonObjectRequest(Method.GET, "$URL_GET_USER${App.prefs.userEmail}", null, Response.Listener {response->
                try {
                    UserDataService.name = response.getString("name")
                    UserDataService.email =  response.getString("email")
                    UserDataService.avatarName =  response.getString("avatarName")
